...
export class ReorderableRow implements StyleableUIElement {
    ...
    public append(element : HTMLElement, dragHandle : HTMLElement) {
        if (this.content.length > 0) {
            this.content[this.content.length - 1].handleToTheRight.setNextDiv(element);
        }
    ...
    public remove(element : HTMLElement) {
        const index = this.content.findIndex((content) => content.element == element);
        const popped = this.content.splice(index, 1)[0];
        if (index > 0) {
            this.content[index-1].handleToTheRight.setNextDiv(index < this.content.length ? this.content[index].element : null);
        }
        popped.element.remove();
        popped.handleToTheRight.remove();
        const leftContent = index > 0 ? this.content[index - 1] : null;
        const rightContent = index < this.content.length ? this.content[index] : null;
     ...
     */
    public getContentByMousePosition(x : number, y : number) : ReorderableRowContent | null {
        for (let elementAndDragHandle of this.content) {
            const boundingRectangle = elementAndDragHandle.dragHandle.getBoundingClientRect();
            if (boundingRectangle.left <= x && boundingRectangle.right >= x && boundingRectangle.top <= y && boundingRectangle.bottom >= y) {
