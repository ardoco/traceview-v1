 ...
 */
function fabricatePickerRow(parent : HTMLElement, label : string, style : Style, options : string[], fontSizeInPx : number, allowEmpty : boolean, onChange : (selected : string | null) => void) : Picker {
 ...
 */
function createATitledPanel(titleLabel : string, style : Style, fontSizeInPx : number) : [HTMLElement,HTMLElement] {
 ...
 */
function createACloseButton(parent : HTMLElement, style : ButtonStyle, fontSizeInPx : number, clickAction : () => void) {
 ...
 */
export function fabricateNewOOPVisPopupPanel(visFactory : VisualizationFactory, 
        sendToApp: (data: { visType: VisualizationType; artifactData: string[]}) => boolean,
        appFileManager : FileManager, style : Style): void {
            const fontSize = 25;
            const popupAndOverlay = createATitledPanel("New Visualization:", style, fontSize);
            ...
            const typeRow = fabricatePickerRow(popup, "Type", style, visFactory.getAllVisualizationTypes().map((type) => visFactory.getTypeName(type)), fontSize, true, (typeName) => {
                artifactsPanel.innerHTML = "";
                inputFilePickers = [];
                if (typeName != null) {
                    for (let i = 0; i < visFactory.getExpectedFileCount(visFactory.getType(typeName)); i++) {
                        const pick = fabricatePickerRow(artifactsPanel, "Input File " + (i+1), style, appFileManager.getAllFileNames(), fontSize, true, (fileName) => {
                        });
            ...
            createACloseButton(popup, style.getButtonStyle(), fontSize, () => {
                if (typeRow.getValue() != null && inputFilePickers.every((picker) => picker.getValue() != null)) {
                    sendToApp({ visType: visFactory.getType(typeRow.getValue()!), artifactData: inputFilePickers.map((picker) => picker.getValue()!)});
                }
                overlay.remove();
            });
}
 ...
 * @returns 
 */
function fabricateSwitchablePairPickerRow(parent : HTMLElement, label : string, pair : [string,string], style : Style, options : string[], fontSizeInPx : number, allowEmpty : boolean, onSwitch : (newState : boolean) => void) : Picker {
    const typeDDM = new Picker(style, options, fontSizeInPx, allowEmpty);
    const typeRow = document.createElement('div');
    typeRow.style.width = "100%";
    ...
    labelDiv.style.textAlign = "right";
    labelDiv.style.paddingRight = "20px";
    labelDiv.appendChild(document.createTextNode(label + " " + pair[0] + " -> " + pair[1] + ":"));
    typeRow.appendChild(labelDiv);
    typeDDM.attachTo(typeRow);
    ...
    switchButton.addEventListener('click', () => {
        ...
        if (switchYes) {
            ...
            labelDiv.appendChild(document.createTextNode(label + " " + pair[1] + " -> " + pair[0] + ":"));
        }
        switchYes = !switchYes;
        onSwitch(switchYes);
    });
