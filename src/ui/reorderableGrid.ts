...
export class ReorderableGrid implements StyleableUIElement {
    ...
    constructor(viewport : HTMLElement, style : Style) {
        ...
        this.viewport.addEventListener("mouseup", (event) => {
            if (this.dragIndicator == null || this.dragStartContent == null || this.dragStartRow == null) {
                return;
            }
            let dragEndRow : ReorderableRow | null = null;
            for (let row of this.rows) {
                const rect = row.getRow().getBoundingClientRect();
                if (this.dragIndicator != null && event.clientX >= rect.left && event.clientX <= rect.right && event.clientY >= rect.top && event.clientY <= rect.bottom) {
                    dragEndRow = row;
                    if (this.dragStartRow != dragEndRow && this.dragStartContent != null) {
                        ...
                        dragEndRow.append(this.dragStartContent.element, this.dragStartContent.dragHandle);
                    }
            ...
            if (dragEndRow == null) {
                this.addRow(0.9);
                for (let row of this.rows) {
                    row.getRow().style.height = (0.9 * this.viewport.getBoundingClientRect().height / this.rows.length) + "px";
                }
    ...
    private addRow(rowHeightFraction : number) {
        if (this.rows.length > 0) {
            this.handles.push(new YResizingHandle(this.rows[this.rows.length-1].getRow(), this.style));
            this.viewport.appendChild(this.handles[this.handles.length-1].getHandle());
            const newRow = new ReorderableRow(this.viewport, this.viewport.getBoundingClientRect().height * rowHeightFraction, this.style);
    ...
    private setupDragAndDropForRow(row : ReorderableRow) {
        ...
        row.getRow().addEventListener("mousedown", (event) => {
            this.dragStartContent = row.getContentByMousePosition(event.clientX, event.clientY);
    ...
    private removeRow(index : number, height : number = 0) {
        ...
        this.rows[0].getRow().style.height = this.rows[0].getRow().getBoundingClientRect().height + height + "px";
        this.handles[index-1].getHandle().remove();
        this.handles.splice(index-1,1);
        for (let row of this.rows) {
            row.getRow().style.height = 0.95 * this.viewport.getBoundingClientRect().height / this.rows.length + "px";
        }
    }
