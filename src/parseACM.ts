...
export function parseCodeFromACM(content : string) : CodeModel {
    ...
    for (let key of Object.keys(json.codeItemRepository.repository)) {
        ...
        typeInstances.set(json.codeItemRepository.repository[key].type, typeInstances.get(json.codeItemRepository.repository[key].type).concat(json.codeItemRepository.repository[key]));
    }
    ...
    for (let controlElement of typeInstances.get("ControlElement")) {
        controlElements.set(controlElement.id, new ACMControlElement(controlElement.id, controlElement.name));
    }
    for (let clazz of typeInstances.get("ClassUnit")) {
        classes.set(clazz.id, new ACMClassUnit(clazz.id, clazz.name, clazz.content.map((id : string) => controlElements.get(id)!)));
    }
    for (let inter of typeInstances.get("InterfaceUnit")) {
        interfaces.set(inter.id, new ACMInterfaceUnit(inter.id, inter.name, inter.content.map((id : string) => controlElements.get(id)!)));
    }
    for (let codecompilationunit of typeInstances.get("CodeCompilationUnit")) {
        const content = []
        for (let contentId of codecompilationunit.content) {
            content.push(json.codeItemRepository.repository[contentId].type == "ClassUnit"  ? classes.get(contentId)! : interfaces.get(contentId)!);
        }
        codeCompilationUnits.set(codecompilationunit.id, new ACMCodeCompilationUnit(codecompilationunit.id, codecompilationunit.name + "." + codecompilationunit.extension, content));
