...
export class UIFactory {
  ...
  public static fabricateHeader(
    ...
    header.style.minHeight =
      HEADER_SIZE_AS_FRACTION_OF_FONT_SIZE * fontSizeinPx + "px";
  ...
  public static attachButtons(
    ...
    buttonPanel.style.width =
      subject.getButtons().length * (headerSize - gap + gap / 8) + "px";
  ...
  public static fabricatePanel<T extends Buttoned>(
    ...
    const buttons: StyleableButtonElement[] = UIFactory.attachButtons(
      buttonPanel,
      40,
      visualization,
   ...
   * @returns
   */
  public static fabricatePageHeaderDropdownButton(
    symbols: string[],
    options: string[],
    actions: (() => void)[],
    fontSizeInPx: number,
    ...
    const setButtonActive = (active: boolean) => {
      button.innerHTML = active
        ? symbols[0]
        : symbols[Math.max(symbols.length - 1, 0)];
    ...
    button.addEventListener("mouseenter", () => {
      ...
      const dropdown = UIFactory.fabricatePageHeaderDropdown(
        options,
        actions,
        width,
   ...
   */
  public static fabricatePageHeaderDropdown(
    options: string[],
    actions: (() => void)[],
